...
var g = (function(gamma) {
		...
		function googleAnalyse() {
			...
			ga('set', {
				...
				title: 'Embold - '+historyManager.get('currentSubSystemName')+' : '+historyManager.get('currentPlugin').name
			});
		...
		function updateSubsystemData() {
			...
			$(document).off('click', '#run_analysis_container .available_scan_credit .upgrade-plan-details');
			$(document).on('click', '#run_analysis_container .available_scan_credit .upgrade-plan-details', function () {
				window.open('https://embold.io/pricing-cloud', '_blank');
			});

			$(document).off('click', '#run_analysis_container .available_scan_credit .scan_credit_label');
			$(document).on('click', '#run_analysis_container .available_scan_credit .scan_credit_label', function () {
				g.hideTreePanel();
				e.changeMode('management');
				...
				$(document).attr('title', 'Embold - ' + i18next.t("common.page_title.admin_details") + ' : ' + i18next.t("admin.license"));
			});
		...
		function setHistoryData(selectedSubsystem) {
			return new Promise(function (resolve, reject) {
				try {

					historyManager.set('currentContext','subsystems');
					selectedRepository = selectedSubsystem;
		...
		function onSubsystemChange(selectedSubsystem) {
			setHistoryData(selectedSubsystem).then(function(){
                ...
                gamma.getLicenseDetails()
					.then(license_summary => {
						if (license_summary.scan_status_flag) {
							...
							if ((g.isOs())){
								$('header .btn-container .available_scan_credit .upgrade-plan-details').html('Need to scan more?');
							}
                        }
						else {
							$('header .btn-container .available_scan_credit .scan_credit_value').html('Unlimited');
							$('header .btn-container .available_scan_credit .upgrade-plan-details').remove();
                        }
				});
			}
